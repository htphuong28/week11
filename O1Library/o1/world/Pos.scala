...
final case class Pos(val x: Double, val y: Double) derives CanEqual:
  ...
  @targetName("plus")
  inline infix def +(another: Pos): Pos = this.add(another)
  ...
  /** Subtracts the two given values from the the `x` and `y` of this `Pos`, respectively. Returns the resulting `Pos`. */
  inline def subtract(dx: Double, dy: Double) = Pos(this.x - dx, this.y - dy)
  /** Subtracts the `x` and `y` of the given `Pos` from the `x` and `y` of this `Pos`, respectively. Returns the resulting `Pos`.
    * The expressions `pos1 - pos2` and `pos1.subtract(pos2)` are equivalent. */
  @targetName("minus")
  inline infix def -(another: Pos): Pos = this + -another
  ...
  @targetName("times")
  inline infix def *(factor: Double): Pos = this.multiply(factor)
  ...
  inline infix def /(divisor: Double): Pos = this.divide(divisor)
  ...
  /** Returns a `String` description of the `Pos` that is identical to [[this.toString]]
  ...
  inline private def copy(x: Double = this.x, y: Double = this.y) = Pos(x, y)


  private[o1] inline def centerBetween(another: Pos): Pos =
    if this == another then this else (this + another) / 2

  // Scales this position relative to a given point of reference.
  private[o1] inline def scaleBy(horizontalFactor: Double, verticalFactor: Double, pointOfReference: Pos): Pos =
    val xDistance = x - pointOfReference.x
    val yDistance = y - pointOfReference.y
    Pos(pointOfReference.x + horizontalFactor * xDistance,
        pointOfReference.y + verticalFactor   * yDistance)

  // Scales the position relative to the origin (0,0) by specified horizontal and vertical factors.
  private[o1] inline def scaleByRelativeToOrigin(horizontalFactor: Double, verticalFactor: Double): Pos =
    scale(this, horizontalFactor, verticalFactor)

  // Rotates the position around the origin (0,0) by a specified angle in degrees.
  private[o1] inline def rotateAroundOrigin(angle: Double): Pos =
    rotate(this, angle)
  ...
  private[o1] inline def rotateAround(pivot: Pos, angle: Double): Pos =
    angle match
      case 0.0    => this
      case 90.0   => rotateBy90DegsCW(this, pivot)
      case 180.0  => rotateBy180Degs(this, pivot)
      case 270.0  => rotateBy90DegsCCW(this, pivot)
      case 360.0  => this
      case -90.0  => rotateBy90DegsCCW(this, pivot)
      case -180.0 => rotateBy180Degs(this, pivot)
      case -270.0 => rotateBy90DegsCW(this, pivot)
      case _      => rotate(this, angle, pivot)
...
object Pos:
  ...
  inline def apply(xyPair: (Double, Double)): Pos = new Pos(xyPair(0), xyPair(1))
